 // EX.NO.11 DEVELOP ACRUD APPLICATION IN JAVA USING JAVAFX AND JDBC

Open the MySQL command line or any MySQL client (like MySQL Workbench) and execute the following commands: 
CREATEDATABASEstudentdb; 
USEstudentdb; 
CREATETABLEstudents ( id INT AUTO_INCREMENTPRIMARYKEY, name VARCHAR(100) NOT NULL, age INT NOTNULL, course VARCHAR(100) NOT NULL );
...
import java.sql.*;   //StudentManagementApp extends Application
public class EX_NO_11 { 
	private Connection connection; private TextField idField = new TextField(); 
	...
	public void start(Stage primaryStage) { 
		...
		ResultSet rs = stmt.executeQuery(sql)) { 
			...
			while (rs.next()) { 
				sb.append("ID: ").append(rs.getInt("id")) .append(", Name: ").append(rs.getString("name")) .append(", Age: ").append(rs.getInt("age")) .append(", Course: ").append(rs.getString("course")) .append("\n"); } displayArea.setText(sb.toString()); 
			} 
		catch (SQLException e) { 
			e.printStackTrace(); 
		} 
